...
[ "$(uname -s)" = "Linux" ] || error "The musl static build is only available for Linux."
case "$(uname -m)" in
  x86_64|amd64) ;;
  *) error "The musl static build is only available for Linux x64." ;;
esac

zip_name="stellar-linux-x64-musl-static.zip"
...
info "Trail musl static build installed successfully to $DST/trail"
